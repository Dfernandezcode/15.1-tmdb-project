...
const MediaInfo = ({ itemData }) => {
  ...
  const genreMap = itemData?.genres.map((genre) => (
    <div className="media-info__text-wrap-genre">
  ...
  return (
    <div className="media-info">
      <img
        className="media-info__poster"
        src={`https://image.tmdb.org/t/p/w440_and_h660_face/${itemData?.poster_path}`}
        alt={itemData?.title}
      ></img>
      <div className="media-info__text-wrap">
        <h2 className="media-info__text-wrap-title">
          {itemData?.title || itemData?.name}
        </h2>
        <h2 className="media-info__text-wrap-year">({year})</h2>
        <div className="media-info__text-wrap-detail">
          <p>{itemData?.release_date || itemData?.first_air_date}</p>{" "}
      ...
      <div className="media-info__crew" castData={creditData}>
        {crewList}
      </div>
    </div>
  );
